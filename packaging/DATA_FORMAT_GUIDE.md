...
if column_range is not None:
    start, end = column_range  # 1-based
    start_idx = start - 1  # 转换为 0-based
    end_idx = end  # 切片结束索引（不包含）
    data = data[:, start_idx:end_idx]  # 只保留指定范围的列
    # 检查切片后的数据是否包含空列
```
...
- 程序支持选择处理文件中的部分列（样本）
- 在数据面板中选择"自定义列范围"模式
- 输入起始列号和结束列号（1-based，与 Excel 列号一致）
- 程序会自动检测空列并提示错误
- **示例**：如果文件有 10 列，输入 1-3 表示只处理前 3 列

**注意事项**：
- 列号从 1 开始（1-based），不是从 0 开始
- 如果起始和结束相同（如 5-5），表示只处理单列
- 空列会被自动检测，如果选择范围包含空列，程序会阻止执行
