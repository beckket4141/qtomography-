...
class DensityMatrix:
    ...
    def is_hermitian(self) -> bool:
        ...
        return np.allclose(self._matrix, self._matrix.conj().T, atol=self.tolerance)
    
    def is_positive_semidefinite(self) -> bool:
        """检查密度矩阵是否为正半定矩阵"""
        eigenvals = self.eigenvalues
        return bool(np.all(eigenvals >= -self.tolerance))
    
    def is_normalized(self) -> bool:
        """检查密度矩阵是否归一化（迹为1）"""
        return bool(np.isclose(self.trace, 1.0, atol=self.tolerance))
    
    def is_physical(self, tol: Optional[float] = None) -> bool:
        ...
        return (vals.min() >= -tol) and (abs(np.trace(self._matrix).real - 1.0) <= tol)
    
    def physical_diagnostics(self, tol: Optional[float] = None) -> dict:
        """
        返回密度矩阵的物理性诊断信息
        
        Args:
            tol: 容差，默认使用实例的 tolerance
        ...
        return {
            ...
            "hermitian_residual": float(np.linalg.norm(self._matrix - self._matrix.conj().T)),
            "tol": float(tol),
            "is_physical_within_tol": bool((vals.min() >= -tol) and (abs(np.trace(self._matrix).real - 1.0) <= tol)),
